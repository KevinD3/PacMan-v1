...
void Player::render(){
    ...
    ofDrawBitmapString("Score: " + to_string(score), 200, 50);
    ofDrawBitmapString("Pacman Lives: " + to_string(health), 650, 50);
    right.draw(800,40);
    ...
    for(Entity* entity:em->entities){
        if(collides(entity)){
            if(dynamic_cast<Dot*>(entity)){
                entity->remove = true;
                score+=15; //15 vale el peq
            }
            else if (dynamic_cast<BigDot*>(entity)){
                entity->remove = true;
                score+=30; //30 vale el grande
            }
        }
    }
    for (Entity*entity:em->entities){
