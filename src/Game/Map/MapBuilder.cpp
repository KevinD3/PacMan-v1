...
MapBuilder::MapBuilder(){
	...
	ghostPink = ofColor(255,0,255);
	ghostBlue = ofColor(0,255,255);
	ghostOrange = ofColor(255,102,0);
    ...
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBlock){
                ...
                GhostSpawner* ghost = new GhostSpawner(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, pacmanSpriteSheet);
                mapInCreation->setGhostSpawner(ghost);
			}else if(currentPixel == ghostPink){
                GhostSpawner* ghost = new GhostSpawner(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, pacmanSpriteSheet);
                mapInCreation->setGhostSpawner(ghost);
			}else if(currentPixel == ghostBlue){
                GhostSpawner* ghost = new GhostSpawner(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, pacmanSpriteSheet);
                mapInCreation->setGhostSpawner(ghost);
			}else if(currentPixel == ghostOrange){
                GhostSpawner* ghost = new GhostSpawner(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, pacmanSpriteSheet);
                mapInCreation->setGhostSpawner(ghost);
